...
import { useFirebaseTimesheets, fmt12, toYMD, fmtHours } from '../../hooks/useFirebaseTimesheets'

export default function MyAttendance() {
  const currentUser = useAppSelector(s => s.auth.user)
  const { employees } = useFirebaseEmployees()
  const myEmployee    = employees.find(e => e.email === currentUser?.email)
  const { entries, attendance } = useFirebaseTimesheets(myEmployee?.id ?? '')
  ...
  const dayStatus = new Map<string, 'present' | 'late' | 'absent' | 'weekend'>()
  for (let d = 1; d <= daysInMo; d++) {
    ...
    if (hrRec) {
      dayStatus.set(dateStr, hrRec.status === 'late' ? 'late' : hrRec.status === 'absent' ? 'absent' : 'present')
    } else if (entries.some(e => e.date === dateStr && !e.clockedIn)) {
      dayStatus.set(dateStr, 'present')
    }
  }

  // Monthly stats
  let present = 0, late = 0, absent = 0
  dayStatus.forEach(s => { if (s === 'present') present++; else if (s === 'late') late++; else if (s === 'absent') absent++ })
  const workDays      = present + late + absent
  const attendancePct = workDays > 0 ? Math.round(((present + late) / workDays) * 100) : 100

  const monthStr    = `${year}-${String(month + 1).padStart(2, '0')}`
  const totalHours  = entries.filter(e => e.date.startsWith(monthStr) && !e.clockedIn).reduce((s, e) => s + e.hours, 0)

  // Selected day entries
  const selEntries = selDay ? entries.filter(e => e.date === selDay) : []
  ...
  const dayClass = (status: string | undefined, isToday: boolean, isSel: boolean) => {
    let base = `aspect-square text-[11px] rounded-lg flex items-center justify-center transition-all ${isSel ? 'ring-2 ring-primary ring-offset-1' : ''} ${isToday ? 'ring-1 ring-primary/50' : ''}`
    if (status === 'present') return `${base} bg-green-100 text-green-700 hover:bg-green-200 cursor-pointer`
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Your attendance history and punch records</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="card p-5 flex-1">
            ...
            {selEntries.length === 0 ? (
              <p className="text-xs text-gray-400 mt-2">No records for this day.</p>
            ) : (
                ...
                {selEntries.map(e => (
                  <div key={e.id} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
                    <div>
                      ...
                      {e.projectTask && <p className="text-[10px] text-gray-400">{e.projectTask}</p>}
                    </div>

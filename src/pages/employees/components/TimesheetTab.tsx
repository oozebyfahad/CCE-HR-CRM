...
// Payable hours — always computed from raw timestamps so overtime is
// never included regardless of what RotaCloud stores in att.hours:
//   - in clipped to shift start  → early arrival not paid
  ...
  const handleApproveAll = async () => {
    ...
    try {
      ...
      for (const att of attendance) {
        ...
        // Detect overtime for notification only
        const shift = shiftByDate.get(d)

        // Payable = clocked hours minus overtime beyond shift end
        approvedHours += clockedHours(att, shift)
...
function TimesheetDetailView({ emp, canApprove }: { emp: FirebaseEmployee; canApprove: boolean }) {
  ...
  const handleSubmitToPayroll = async () => {
    ...
    try {
      ...
      for (const date of localApproved) {
        ...
        shiftCount++
        // Payable = clocked hours minus overtime beyond shift end
        totalPayable += clockedHours(att, shift)
  ...
  const approvedPayable = [...localApproved].reduce((s, date) => {
    const att = attByDate.get(date); const shift = shiftByDate.get(date)
    return s + (att ? clockedHours(att, shift) : 0)
  }, 0)
        ...
        <table className="w-full text-xs min-w-[960px] border-collapse">
          ...
          <tbody>
            {days.map(({ date, att, shift }) => {
              ...
              const payable = att ? clockedHours(att, shift) : 0

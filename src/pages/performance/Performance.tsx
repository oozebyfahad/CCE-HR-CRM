...
export default function Performance() {
  const [selected, setSelected] = useState(mockReviews[0])
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="page-sub">Q1 2026 review cycle</p>
        </div>
        <button className="btn-primary text-sm">+ New Review</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {mockReviews.map(r => (
              <div key={r.id} onClick={() => setSelected(r)} className={`p-4 cursor-pointer hover:bg-primary-50/30 transition-colors ${selected.id === r.id ? 'bg-primary-50/50' : ''}`}>
